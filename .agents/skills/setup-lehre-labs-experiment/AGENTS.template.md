...
{agent-write: Pointers an agent should know before working here -- the golden/reference files, key entries under `docs/`, plus any caveats, gotchas, or short architecture notes. Keep it concrete and current; delete what goes stale. Keep this lean, and verbose things go under `docs/`.}
